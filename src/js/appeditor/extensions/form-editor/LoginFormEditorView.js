    ...
    function (FormFieldModel, ActionEditorView, TutorialView) {
        ...
        var LoginFormEditorView = Backbone.ModalView.extend({
            ...
            render: function () {
                ...
                var pages = _.map(v1State.get('pages').getContextFreePages(), function (page) {
                    return {
                        name: page,
                        val: 'internal://' + page
                    };
                });
                var userPages = _.map(v1State.get('pages').getPagesWithEntityName('User'), function (page) {
                    return {
                        name: page,
                        val: 'internal://' + page + '/?User=CurrentUser'
                    };
                });
                pages = _.union(pages, userPages);
                this.model.each(function (route) {
                    this.el.innerHTML += _.template(FormEditorTemplates.routeTemplate, {
                        route: route,
                        pages: pages
                    });
                }, this);
